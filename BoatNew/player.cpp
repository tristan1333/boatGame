...
{
	std::string execCod = "tiles.reg['" + mp->tiles[x][y]->type->id + "']:onEnter(" + std::to_string(x) + ", " + std::to_string(y) + ", " + std::to_string(lx) + ", " + std::to_string(ly) + ", " + std::to_string(dir) + ")";
...
//	std::cout << "starting at" << (int)this->pos->x;
	...
	std::string execCode = std::string("player:setPos(" + std::to_string(this->pos->x) + ", " + std::to_string(this->pos->y) + ")");
