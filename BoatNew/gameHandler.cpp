...
void game::handle_game(const Uint8 *key, camera* maincam, tilemap* mp, player * plr, LuaContext * L, eventHandler* events)
{
	for (int y = 0; y <= 14; y++)
	{
	...
	std::string oss = std::string("") + std::to_string((int)(plr->pos->x + plr->w)) + ", " + std::to_string((int)plr->pos->y);
...
void game::handle_game(camera* maincam, const Uint8 *key, map* mp)
{
	...
	//convertToWorld(nx, ny);
//	std::string oss = std::to_string(nx);
	std::string oss = std::string("") + std::to_string(mousex + (int)maincam->camPos.x) + ", " + std::to_string((int)maincam->camPos.y + ny);
