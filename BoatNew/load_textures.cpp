...
void renAngle(SDL_Renderer* ren, SDL_Texture* tex, double x, double y, double w, double h, double angle, SDL_Point* ang_pnt = NULL, SDL_RendererFlip flip = SDL_FLIP_NONE)
{
	SDL_Rect dst;
	dst.x = x;
	dst.y = y;
...
bool init(resHandler * res) //Starts up the SDL library and makes sure everything is working. 
{
	...
	{
		mainwin = SDL_CreateWindow("Project", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, res->w * res->resMod, res->h * res->resMod, SDL_WINDOW_SHOWN);
		if(mainwin == NULL)
		{
			success = false;
		}
