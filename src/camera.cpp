...
void convertToWorld(float &x, float &y,
                    camera *TargCam) //We simply want to take screen coordinates, and convert them to the world.
{
    //std::cout << TargCam->camPos.x;
    x = (x  /*(TargCam->camPos.x * TargCam->res->wldToScrx)*/) / TargCam->res->wldToScrx /
        (TargCam->res->w / (TargCam->w * TargCam->res->wldToScrx));
...
void renderToWorld(Vec2 *pos, float w, float h, camera *TargCam, SDL_Texture *tex) {
    ...
    renSpec(mainren, tex, w * TargCam->res->wldToScrx * (TargCam->res->w / (TargCam->w * TargCam->res->wldToScrx)),
            h * TargCam->res->wldToScry * (TargCam->res->h / (TargCam->h * TargCam->res->wldToScry)),
            ((pos->x * TargCam->res->wldToScrx) - (TargCam->camPos.x * TargCam->res->wldToScrx)) *
            (TargCam->res->w / (TargCam->w * TargCam->res->wldToScrx)),
            ((pos->y * TargCam->res->wldToScry) - (TargCam->camPos.y * TargCam->res->wldToScry)) *
            (TargCam->res->h / (TargCam->h * TargCam->res->wldToScry)));
