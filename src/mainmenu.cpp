...
int mainMenu::handle(resHandler *res) {
    ...
    SDL_Texture *text_curr = loadText("Play Game", col, res->w / 10 * res->resMod, res->h / 8 * res->resMod, tx_wd,
                                      tx_h);
    SDL_DestroyTexture(text_curr);
    text_curr = loadText("Edit Map", col, res->w / 10 * res->resMod, res->h / 8 * res->resMod + tx_h + (tx_h / 2),
                         tx_wd, tx_h);
    SDL_DestroyTexture(text_curr);
    return 1;
}